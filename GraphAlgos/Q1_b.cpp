...
void bfs(state sc){
    dist = vector<vector<vector<int>>>(n,vector<vector<int>>(m,vector<int>(k_limit+1,INF)));
    visited = vector<vector<vector<int>>>(n,vector<vector<int>>(m,vector<int>(k_limit+1,0)));
    //setup DS
    queue<state> q;
